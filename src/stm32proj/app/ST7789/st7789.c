...
#endif
#ifdef USE_LL
void SPI1_BasicWrite(uint8_t byte)
{
    LL_SPI_Enable(SPI1);
...
void SPI1_ReadWriteBytes(uint8_t *pTxData, uint32_t Count)
{
    for (uint32_t i = 0; i < Count; i++)
    {
        SPI1_BasicWrite(pTxData[i]);
    }
}
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	while (buff_size > 0) {
		...
		#ifdef USE_LL
			SPI1_ReadWriteBytes(buff,chunk_size);
		#else
			#ifdef USE_DMA
...
static void ST7789_WriteSmallData(uint8_t data)
{
	...
	ST7789_DC_Set();
	#ifdef USE_LL
		SPI1_ReadWriteBytes(&data,sizeof(data));
	#else
